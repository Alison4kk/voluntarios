...
@php
    ...
    $placeholder_color =
        $show_placeholder_always || $label == '' ? '' : 'placeholder-transparent dark:placeholder-transparent';
    ...
    if ($clearable) {
        ...
        $suffix_is_icon = true;
        $suffix_icon_css =
            'hidden cursor-pointer dark:!bg-dark-900/60 dark:hover:!bg-dark-900 !p-0.5 !rounded-full bg-gray-400 !stroke-2 hover:bg-gray-600 text-white';
    }

    if ($attributes->has('readonly') || $attributes->has('disabled')) {
        if ($attributes->get('readonly') == 'false') {
            $attributes = $attributes->except('readonly');
        }
        if ($attributes->get('disabled') == 'false') {
            $attributes = $attributes->except('disabled');
        }
    }
@endphp
...
<div class="relative w-full dv-{{ $name }} @if ($add_clearing) mb-4 @endif">
    ...
    @endif
    @if (!empty($label))
        <label for="{{ $name }}" class="form-label {{ $size }}"
